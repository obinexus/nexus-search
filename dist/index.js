    ...
    async addDocuments(documents) {
        ...
        try {
            ...
            for (const doc of documents) {
                ...
                const searchableContent = createSearchableFields({ content: doc.fields, id: docId }, this.config.fields);
                for (const field of this.config.fields) {
            ...
            await this.indexManager.addDocuments(Array.from(this.documents.values()).map(doc => doc.toObject()));
            try {
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            const enhancedResults = results.map(result => {
                ...
                return ({
                    ...result,
                    document: ((_a = this.documents.get(result.id)) === null || _a === void 0 ? void 0 : _a.toObject()) || result.document,
                    metadata: {
                        ...result.metadata,
                        lastAccessed: Date.now()
                    }
...
export { CacheManager, CacheStrategyType, DataMapper, IndexError, IndexManager, IndexMapper, IndexedDB, NexusSearch, PerformanceMonitor, QueryProcessor, SearchEngine, SearchError, StorageError, TrieNode, TrieSearch, ValidationError, createSearchableFields, NexusSearch as default, getNestedValue, isIndexConfig, isSearchOptions, isSearchResult, normalizeFieldValue, optimizeIndex, validateDocument, validateIndexConfig, validateSearchOptions };
